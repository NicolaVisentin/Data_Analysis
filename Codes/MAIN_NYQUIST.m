...
subplot(2,1,2)  % phase
plot(f_pos,unwrap(rad2deg(angle(X_pos))))
grid on
xlabel('f [Hz]')
ylabel('∠X  [deg]')
axis tight
...
subplot(2,1,2)  % phase
plot(f_pos,unwrap(rad2deg(angle(Y_pos))))
grid on
xlabel('f [Hz]')
ylabel('∠Y  [deg]')
axis tight
...
T=45;       % assign time width of the subrecords
...
subplot(2,1,2)  % phase
plot(f_T_pos,unwrap(rad2deg(angle(Gxy))))
xlabel('f [Hz]')
ylabel('∠G_x_y  [deg]')
grid on
axis tight
...
subplot(2,1,2)  % phase
plot(f_T_pos,unwrap(rad2deg(angle(H1_pos))))
grid on
xlabel('f [Hz]')
ylabel('∠H_1  [deg]')
axis tight
...
subplot(2,1,2)  % phase
plot(f_T_pos,unwrap(rad2deg(angle(H2_pos))))
grid on
xlabel('f [Hz]')
ylabel('∠H_2  [deg]')
axis tight
...
% Check applicability of SDOF method, using Nyquist

    % Estrai la parte reale e immaginaria
    real_part = real(H1_range);
    imag_part = imag(H1_range);
    
    % Crea il Nyquist plot
    figure;
    plot(real_part, imag_part, 'b-', 'LineWidth', 1.5);
    hold on;
    plot(real_part, -imag_part, 'r--', 'LineWidth', 1.5); % Specchio rispetto all'asse reale
    hold off;
    grid on;
    axis equal;
    xlabel('Re');
