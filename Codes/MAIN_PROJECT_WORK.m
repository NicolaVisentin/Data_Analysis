...
% load data: we expect 2 records: random input (first in the 
...
% convert from voltage to force (actually we'll use current as input,
% which is assumed to be proportional to the force) and acceleration 
% using sensitivities
...
subplot(2,1,1)  % plot input force (current)
plot(t,x)
grid on
...
% plot input in frequency domain
...
grid on
title('Choose the frequency range to analyse')
xlabel('f [Hz]')
